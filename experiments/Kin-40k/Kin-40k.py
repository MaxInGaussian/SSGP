...
def load_kin40k_data(proportion=0.75):
    kin40k = np.loadtxt("Dataset.data", dtype=np.float64)
    from sklearn import cross_validation
    X, y = kin40k[:, :-1], kin40k[:, -1]
    y = y[:, None]
...
f, axarr = plt.subplots(nrows=2, ncols=2, figsize=(12, 8), dpi=200)
maxv, minv = 0, 1e5
for i, penalty in enumerate(penalties):
